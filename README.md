...
| `contracts` (run-level) | List of symbol names no lane may redefine — an added line defining one (`class`, `def`, `struct`, `enum`, `interface`, `type`, `typedef`, `protocol`, `func`, `fn`, `const`, `let`, `var`) fails that lane with the symbol, file, and line named. CSS custom properties count when the contract includes the leading dashes (`--accent`). The foundation task itself is exempt |
...
CSS custom properties are covered too, but you must include the leading dashes in the contract: `"--accent"`, not `"accent"`. A custom property is declared as `--accent: value` with no keyword in front of it, so the keyword list above cannot see one — without the dashes the contract silently passes and a lane can redefine your design token unnoticed. Using a token (`var(--accent)`) is never a violation; only redefining it is.
