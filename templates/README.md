...
| `red-team` | Runs four read-only adversary lanes against the running product — real-data, destructive-path, error-path, test-quality — each filing evidence-backed findings with a Silent flag. | You need to harden a green build by exercising the running product before calling it done. | Proven in a recorded run, 2026-08-14 — audited Ringer itself and found a false-green in this kit's own check |
